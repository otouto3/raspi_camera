...
k_list = ['K-chessboard.csv', 'K1280720.csv', 'K640480.csv']
d_list = ['d-chessboard.csv', 'd1280720.csv', 'd640480.csv']

if tkMessageBox.askyesno('askyesno', 'キャリブレーションデータ(K.csv, d.csv)を読み込みますか？'):
    # キャリブレーションデータの読み込み
    camera_mat = np.loadtxt(k_list[0], delimiter=',')
    dist_coef = np.loadtxt(d_list[0], delimiter=',')
    print("K = ¥n", camera_mat)
    ...
    while(True):
        ...
        found, corner = cv2.findChessboardCorners(
            grayscale_image, grid_intersection_size)
        ...
        if found == True:
            ...
            cv2.drawChessboardCorners(
                frame, grid_intersection_size, corner, found)
        if found == False:
            print('findChessboardCorners : False')

        cv2.putText(frame, "Enter:Capture Chessboard(" + str(capture_count) + ")",
                    (100, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
        cv2.putText(frame, "N    :Completes Calibration Photographing",
                    (100, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
        cv2.putText(frame, "ESC  :terminate program", (100, 100),
    ...
    if len(image_points) > 0:
        ...
        print("K = \n", K)
        print("d = ", d.ravel())
        np.savetxt("K-chessboard.csv", K, delimiter=',',
                   fmt="%0.14f")  # カメラ行列の保存
        np.savetxt("d-chessboard.csv", d, delimiter=',',
                   fmt="%0.14f")  # 歪み係数の保存
        ...
        for i in range(len(object_points)):
            image_points2, _ = cv2.projectPoints(
                object_points[i], r[i], t[i], camera_mat, dist_coef)
            error = cv2.norm(
                image_points[i], image_points2, cv2.NORM_L2) / len(image_points2)
            mean_error += error
